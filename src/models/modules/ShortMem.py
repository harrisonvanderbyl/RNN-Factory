...
class Short_Mem(nn.Module):
    def __init__(self, args, layer_id, shiftAmount=1):
        super().__init__()
    ...
    def forward(self, x):
        B, T, C = x.size()
        ks=[self.time_shift1(x),x]

        ct = torch.cat(ks, dim=-1)
...
class WaveNet_Mem(Short_Mem):
    def __init__(self, args, layer_id, modulo=12, undialated=False, cap=12):
        if undialated:
            super().__init__(args, layer_id,  shiftAmount=1)
        else:
            super().__init__(args, layer_id, shiftAmount=2**((layer_id%modulo) if layer_id < cap else 0))